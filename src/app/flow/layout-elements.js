...
const entitreeSettings = {
    ...
    firstDegreeSpacing: 140, // spacing in px between nodes belonging to the same source, eg children with same parent
    nextAfterAccessor: "spouses", // the side node prop used to go sideways, AFTER the current node
    nextAfterSpacing: 140, // the spacing of the "side" nodes AFTER the current node
    nextBeforeAccessor: "siblings", // the side node prop used to go sideways, BEFORE the current node
    nextBeforeSpacing: 140, // the spacing of the "side" nodes BEFORE the current node
    nodeHeight, // default node height in px
    nodeWidth, // default node width in px
    orientation: Orientation.Horizontal, // "vertical" to see parents top and children bottom, "horizontal" to see parents left and
    rootX: 0, // set root position if other than 0
    rootY: 0, // set root position if other than 0
    secondDegreeSpacing: 140, // spacing in px between nodes not belonging to same parent eg "cousin" nodes
    sourcesAccessor: "parents", // the prop used as the array of ancestors ids
    sourceTargetSpacing: 140, // the "vertical" spacing between nodes in vertical orientation, horizontal otherwise
    targetsAccessor: "children",
    dragging: true,
    draggable: true,
    type: "dragHandleNode",
    // the prop used as the array of children ids
};

const { Top, Bottom, Left, Right } = Position;

export const layoutElements = (tree, rootId, direction = "LR") => {
    ...
    entitreeEdges.forEach((edge) => {
        ...
        newEdge.reconnectable = false;
        newEdge.focusable = false;
        edges.push(newEdge);
    });
    ...
    entitreeNodes.forEach((node) => {
        ...
        newNode.nodesConnectable = false;
        newNode.elementsSelectable = false;
        // newNode.dragHandle = ".custom-drag-handle";
        // newNode.type = "dragHandleNode";
        newNode.position = {
            x: node.x,
            y: node.y,
        };
